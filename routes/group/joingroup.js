...
async function Joingroup(req, res) {
  ...
  try{
    ...
    if(isMember){
        return res.json({ success: false, message: "You are already in the group" });
    }
    Group.findByIdAndUpdate(params,{$addToSet:{members:userid}},
        {new:true,useFindAndModify:false},(err,updatedGroup) =>{
            if(err){
                return res.json({
                    success: false,
